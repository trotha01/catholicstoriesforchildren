module Animations.SongsOfTheSaints.Main exposing (..)

import Animations.Helpers exposing (..)
import Animations.SongsOfTheSaints.SotSEpisodes exposing (..)
import Browser
...
import Task
import Team.Team exposing (viewDescription)
            ...
            case urlRequest of
                Browser.Internal url ->
                    let
                        ...
                        isDASPage =
                            String.contains "songsofthesaints" urlString
                    in
                    if isDASPage then
...
viewEpisode : Model -> SotSEpisode msg -> Html Msg
viewEpisode model episode =
    div
        ...
        , div [ class "mt-10" ] []
        , viewVideoDescriptions model episode |> Html.map (\_ -> NoOp)
        ]
...
episodeUrlParam : SotSEpisode msg -> String
episodeUrlParam episode =
    episode.title |> removeSpaces |> String.toLower


episodeFromURL : String -> Maybe (SotSEpisode msg)
...
viewVideoPlayers : Model -> SotSEpisode msg -> Html Msg
...
viewVideoDescriptions : Model -> SotSEpisode msg -> Html msg
viewVideoDescriptions model page =
    div
        []
        [ case model.videoTab of
            English ->
                viewVideoDescription page.videoDescriptions.english

            Spanish ->
                viewVideoDescription page.videoDescriptions.spanish

            Asl ->
                viewVideoDescription page.videoDescriptions.asl

            Urdu ->
                viewVideoDescription page.videoDescriptions.urdu
        ]


viewVideoPlayerTabs : Model -> SotSEpisode msg -> Html Msg
viewVideoPlayerTabs model page =
    let
        selectedClass =
